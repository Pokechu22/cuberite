...
bool cRoot::ScheduleTaskInWorld(const AString & a_WorldName, int a_DelayTicks, std::function<void(cWorld &)> a_Task)
{
	GetWorld(a_WorldName)->ScheduleTask(a_DelayTicks, a_Task);
	return true;
}
...
bool cRoot::ScheduleTaskInWorld(cWorld & a_World, int a_DelayTicks, std::function<void(cWorld &)> a_Task)
{
	a_World.ScheduleTask(a_DelayTicks, a_Task);
	return true;
}
