...
void cClientHandle::HandleLeftClick(int a_BlockX, int a_BlockY, int a_BlockZ, eBlockFace a_BlockFace, UInt8 a_Status)
{
	...
	if (PlgMgr.CallHookPlayerLeftClick(*Player, a_BlockX, a_BlockY, a_BlockZ, a_BlockFace, static_cast<char>(a_Status)))
	{
...
void cClientHandle::HandleRightClick(int a_BlockX, int a_BlockY, int a_BlockZ, eBlockFace a_BlockFace, int a_CursorX, int a_CursorY, int a_CursorZ, const cItem & a_HeldItem)
{
	...
	auto & PlgMgr = cRoot::Get()->GetPluginManager();
	if (PlgMgr.CallHookPlayerRightClick(*Player, a_BlockX, a_BlockY, a_BlockZ, a_BlockFace, a_CursorX, a_CursorY, a_CursorZ))
	{
		// A plugin doesn't agree with the action, replace the block on the client and quit:
		if (AreRealCoords)
		{
	...
	else if ((ItemHandler->IsFood() || ItemHandler->IsDrinkable(EquippedDamage)))
	{
		...
		Player->StartEating();
		if (PlgMgr.CallHookPlayerEating(*Player))
		{
	...
	{
		if (PlgMgr.CallHookPlayerUsingItem(*Player, a_BlockX, a_BlockY, a_BlockZ, a_BlockFace, a_CursorX, a_CursorY, a_CursorZ))
		{
