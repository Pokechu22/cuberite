...
bool cEntity::Initialize(const std::shared_ptr<cEntity> & a_Entity, cWorld & a_World)
{
...
bool cEntity::OnPreWorldTravel(cWorld & a_NewWorld)
{
	...
	GetWorld()->QueueTask(
		[this, &a_NewWorld](cWorld & a_CurrentWorld)
		{
			...
			if (Entity->IsPawn())
			{
				a_CurrentWorld.ForEachEntity(Callback(std::static_pointer_cast<cPawn, cEntity>(Entity)));
			}
