...
public:
	...
	bool ScheduleTaskInWorld(const AString & a_WorldName, int a_DelayTicks, std::function<void(cWorld &)> a_Task);
	bool ScheduleTaskInWorld(cWorld & a_World, int a_DelayTicks, std::function<void(cWorld &)> a_Task);
	bool QueueTaskInWorld(cWorld & a_World, std::function<void(cWorld &)> a_Task) { return ScheduleTaskInWorld(a_World, 0, a_Task); }
